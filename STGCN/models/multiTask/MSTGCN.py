...
class Unimodal_filter(nn.Module):
    def __init__(self, a_indim, b_indim, hiddendim, outdim, nheads, dropout=0.2):
        super(Unimodal_filter,self).__init__()
        self.alinear = nn.Linear(a_indim, hiddendim)
        self.blinear = nn.Linear(b_indim, hiddendim)
        self.norm = nn.BatchNorm1d(hiddendim)
        self.crossmodal = MultiheadAttention(embed_dim=hiddendim, num_heads=nheads, batch_first=True)
        self.dropout = nn.Dropout(dropout)
        self.shiftlinear = nn.Linear(hiddendim, outdim)
        self.LNlinear = nn.Linear(outdim, 32)

    def forward(self, xa, xb):
        '''
        :param xa: (batchsize , dim)
        :param xb: (batchsize , dim)
        :return:
        '''
        # a = self.norm(torch.relu(self.alinear(xa))).unsqueeze(1)
        # b = self.norm(torch.relu(self.blinear(xb))).unsqueeze(1) # b: (B,1,D)
        a = torch.relu(self.alinear(xa)).unsqueeze(1)
        b = torch.relu(self.blinear(xb)).unsqueeze(1)
        ya_btoa, _ = self.crossmodal(a, b, a)
        ya_btoa = self.dropout(ya_btoa)
        ya = torch.relu(self.shiftlinear(torch.add(a, ya_btoa).squeeze(1))) # a_shiftb + a -> (B,D)
        la = torch.tanh(self.LNlinear(ya))
        return ya, la
...
class MSTGCN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.text_att = ExpressionSelfAttention(self.bert_text_dim, dropout=self.text_prob)
        self.video_att = ExpressionSelfAttention(self.video_in, dropout=self.video_prob)
        self.audio_att = ExpressionSelfAttention(self.audio_in, dropout=self.audio_prob)
