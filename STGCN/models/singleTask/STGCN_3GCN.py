...
class GCNsubnet(nn.Module):
    def __init__(self, in_dim, g_dim, hidden_dim, seq_len, dropout=0.2, heads=4):
        ...
        self.GNN = Sequential('x, edge_index, edge_weight', [
            (GCNConv(in_dim, 4*hidden_dim), 'x, edge_index, edge_weight -> x1'),
            (GCNConv(4*hidden_dim, 2*hidden_dim), 'x1, edge_index, edge_weight -> x2'),
            (GCNConv(2*hidden_dim,  hidden_dim), 'x2, edge_index, edge_weight -> x3'),
        ])
...
## model++
class STGCN_3GCN(nn.Module):

    def __init__(self, args):
        super(STGCN_3GCN, self).__init__()
