...
class STGCN_SELFMM(nn.Module):
    def __init__(self, args):
        super(STGCN_SELFMM, self).__init__()
        # text subnets
        ...
        self.audio_out = self.audio_hidden
        self.video_out = self.video_hidden
        ...
        self.edge_text_dropout = nn.Dropout(p=self.post_text_prob)
        self.t_bilstm = SubNet_mt(2 * self.text_hidden, self.text_hidden, self.text_hidden, dropout=self.audio_prob,
                                 bidirectional=True)
        self.edge_text_layer_1 = nn.Linear(self.text_hidden, self.post_text_dim)
        self.edge_text_layer_2 = nn.Linear(self.post_text_dim, self.post_text_dim)
        self.edge_text_layer_3 = nn.Linear(self.post_text_dim, output_dim)

        # multitask for audio_edge
        self.edge_audio_dropout = nn.Dropout(p=self.post_audio_prob)
        self.a_bilstm = SubNet_mt(2*self.audio_hidden, self.audio_hidden, self.audio_hidden, dropout=self.audio_prob,
               bidirectional=True)
        self.edge_audio_layer_1 = nn.Linear(self.audio_hidden, self.post_audio_dim)
        self.edge_audio_layer_2 = nn.Linear(self.post_audio_dim, self.post_audio_dim)
        self.edge_audio_layer_3 = nn.Linear(self.post_audio_dim, output_dim)

        # multitask for video_edge
        self.edge_video_dropout = nn.Dropout(p=self.post_video_prob)
        self.v_bilstm = SubNet_mt(2*self.video_hidden, self.video_hidden, self.video_hidden, dropout=self.audio_prob,
                                   bidirectional=True)
        self.edge_video_layer_1 = nn.Linear(self.video_hidden, self.post_video_dim)
        self.edge_video_layer_2 = nn.Linear(self.post_video_dim, self.post_video_dim)
        self.edge_video_layer_3 = nn.Linear(self.post_video_dim, output_dim)


        # the post_fusion layers
        self.post_fusion_dropout = nn.Dropout(p=self.post_fusion_prob)
        self.post_fusion_layer_1 = nn.Linear(self.text_out + self.video_out + self.audio_out, self.post_fusion_dim)
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        # text
        x_t = self.edge_text_dropout(lt)
        x_t = self.t_bilstm(x_t)
        x_t = F.relu(self.edge_text_layer_1(x_t), inplace=True)
        x_t = F.relu(self.edge_text_layer_2(x_t), inplace=True)
        e_t = self.edge_text_layer_3(x_t)
        # audio_edge
        x_a = self.edge_audio_dropout(la)
        x_a = self.a_bilstm(x_a)
        x_a = F.relu(self.edge_audio_layer_1(x_a), inplace=True)
        x_a = F.relu(self.edge_audio_layer_2(x_a), inplace=True)
        e_a = self.edge_audio_layer_3(x_a)
        # video_edge
        x_v = self.edge_video_dropout(lv)
        x_v = self.v_bilstm(x_v)
        x_v = F.relu(self.edge_video_layer_1(x_v), inplace=True)
        x_v = F.relu(self.edge_video_layer_2(x_v), inplace=True)
        e_v = self.edge_video_layer_3(x_v)
