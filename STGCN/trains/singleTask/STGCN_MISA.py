        ...
        optimizer = optim.Adam(
            [{"params": bert_params_decay, "weight_decay": self.args.weight_decay_bert,
             ...
             {"params": list(model.Model.V_GCN.parameters()), "weight_decay": self.args.weight_decay_video,
              'lr': self.args.learning_rate_video},
             {'params': filter(lambda p: p.requires_grad, model_params_other), "weight_decay": self.args.weight_decay_other,
        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # M-loss
                    c_loss = 0.0
                    c_loss += eval('self.args.M') * self.criterion(outputs['M'], labels['M'])
                    # text-node-loss
                    c_loss += eval('self.args.T') * self.criterion(outputs['T'], labels['M'])
                    # edge-loss
                    c_loss += eval('self.args.A') * self.criterion(outputs['ea'], labels['M'])
                    c_loss += eval('self.args.V') * self.criterion(outputs['ev'], labels['M'])
                    c_loss += eval('self.args.T') * self.criterion(outputs['et'], labels['M'])
                    # audio-node-loss
                    c_loss += eval('self.args.A') * self.criterion(outputs['A'], labels['M'])
                    # video-node-loss
                    c_loss += eval('self.args.V') * self.criterion(outputs['V'], labels['M'])
                    # common-loss
                    c_loss += eval('self.args.V') * eval('self.args.T') * self.criterion(outputs['V'], outputs['T'])
                    c_loss += eval('self.args.A') * eval('self.args.T') * self.criterion(outputs['A'], outputs['T'])
                    c_loss += eval('self.args.A') * eval('self.args.V') * self.criterion(outputs['A'], outputs['V'])
