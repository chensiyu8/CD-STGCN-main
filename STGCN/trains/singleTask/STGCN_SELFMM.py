        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    loss = 0.0
                    for m in self.args.tasks:
                        loss += eval(f'self.args.{m}') * self.weighted_loss(outputs[m], self.label_map[self.name_map[m]][indexes], \
            ...
            if epochs - best_epoch >= self.args.early_stop:
                if self.args.save_labels:
                    with open(os.path.join(self.args.res_save_dir,
                                           f'{self.args.model_name}-{self.args.dataset_name}-labels.pkl'), 'wb') as df:
